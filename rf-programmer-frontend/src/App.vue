...
<template>
  <div class="flex flex-col items-center justify-center h-screen px-5 text-white">
    ...
    <div v-auto-animate="{ duration: 500 }"
      ...
      <div class="flex justify-between w-full gap-2">
        <button @click="getRandomNumber(10, 2000)"
          class="w-full h-12 px-5 py-2 font-semibold duration-200 border-2 rounded-md bg-sky-600 border-sky-700 bg-opacity-30 hover:bg-sky-700">Generate
          Key</button>
        <button @click="isToggled = true"
          class="w-full h-12 px-5 py-2 font-semibold duration-200 border-2 rounded-md bg-opacity-30 bg-cyan-600 hover:bg-cyan-700 border-cyan-700">Start
          Transmission</button>
      </div>
      ...
      <button v-if="isToggled" @click="isToggled = false"
        class="h-12 px-5 py-2 mx-auto font-semibold duration-200 border-2 rounded-md bg-opacity-30 bg-rose-600 hover:bg-rose-700 border-rose-700">Stop
        Transmission</button>
    </div>
  </div>
</template>
